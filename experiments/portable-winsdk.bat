...
set MANIFEST_OPT=
if exist %~dp0\manifest_17.%SDKVER%.json ( set MANIFEST_OPT=--manifest %~dp0\manifest_17.%SDKVER%.json )
...
echo -- downloading SDK components to tmp/ ...
  bin\xwin -Loff --cache-dir=tmp %MANIFEST_OPT% --sdk-version %SDKVER% download
if errorlevel 1 (echo error invoking xwin winsdk download && exit /b 1)

echo -- unpacking SDK components to %SDKROOT% ...
  bin\xwin --accept-license -Loff --cache-dir=tmp %MANIFEST_OPT% --sdk-version %SDKVER% splat --output %SDKROOT%
