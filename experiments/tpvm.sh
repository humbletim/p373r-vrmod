...
PYTHONEXECUTABLE=${PYTHONEXECUTABLE:-python3}

fail() { echo "$@" | tee -a fails.txt >&2; echo | cat > /dev/null ; exit 1; }
...
case "$CMD" in
    init)
        ...
        clangpp_exe=$(realpath --relative-base=. "$LLVM_DIR/bin" | cat)/clang++
        # Symlinks in CWD
        ...
        # Local winsdk setup
        WINSDK_DIR=${WINSDK_DIR:-$REPO_ROOT/winsdk}
        test -d "$WINSDK_DIR" || {
            WINSDK_DIR=$(dirname "$SNAPSHOT_ARG")/winsdk
            test -d "$WINSDK_DIR" || fail "WINSDK_DIR not found at $WINSDK_DIR"
        }

        # ln -sfn "$WINSDK_DIR/crt" winsdk/crt
        # ln -sfn "$WINSDK_DIR/sdk" winsdk/sdk

        # Determine base
        SNAPSHOT_NAME=$(basename "$SNAPSHOT_ARG" | cat)
        ...
        export snapshot_dir="$SNAPSHOT_ARG"
        export winsdk="$WINSDK_DIR"
        export devtime="env"
        export base="$BASE"

        # Find _llvm include path
        # Assuming llvm is symlinked in CWD now
        _LLVM_INC=$(find $LLVM_DIR/lib/clang -path '*/include' -type d | head -n 1)
        if [ -z "$_LLVM_INC" ]; then
        ...
        # Merge vfsoverlay.json if generated by xwin else use vfsoverlay.extra.json 
        # if [[ -s "$WINSDK_DIR/vfsoverlay.json" ]] ; then
        #     jq -s ".[0] * { roots: (.[0].roots + .[1].roots) }" \
        #         "$REPO_ROOT/experiments/winsdk.in/vfsoverlay.extra.json" \
        #         "$WINSDK_DIR/vfsoverlay.json" > $WINSDK_DIR/_vfsoverlay.json
        # else
        # for x in `make  fs-open.exe  -j 2>&1 | grep -Eo '\w+\.lib' ` ; do y=$( cd ../winsdk && find . -iname $x | sed -e 's@^\./@${winsdk}/@g' ) ; echo '{ "type": "file", "name": "$x", "external-contents": "$y" },' | x=$x y=$y envsubst ; done | xsel -b

            subst <(grep -vE '^\s*//' "$REPO_ROOT/experiments/winsdk.in/vfsoverlay.extra.json") env/_vfsoverlay.json
            cp -av "$REPO_ROOT/experiments/winsdk.in/windows.include_aliases.h" env/
            cp -av "$TEMPLATES_DIR/fs.include_aliases.h" env/
        # fi

        # Generate winsdk rsp files
        subst "$REPO_ROOT/experiments/winsdk.in/winsdk.rsp" "env/winsdk.rsp"
        subst "$REPO_ROOT/experiments/winsdk.in/mm.rsp" "env/mm.rsp"

        subst "$SNAPSHOT_ARG/llobjs.rsp.in" "env/llobjs.rsp"
        subst <(sed -e 's@^-I@-isystem@g' "$SNAPSHOT_ARG/llincludes.rsp.in") "env/llincludes.rsp"
        ...
        declare -p PYTHONEXECUTABLE WINSDK_DIR SNAPSHOT_ARG LLVM_DIR clangpp_exe | tee env/env
...
#         echo "Created env/llpreprocessor_shim.h shim and appended env/compile.rsp to utilize."
    ...
    mod)
        ...
        . env/env || fail "Snapshot symlink not found. Run '$0 init' first."

        # Search in snapshot/source
        FOUND=$(find "$SNAPSHOT_ARG/source" -name "$FILE_PART" | head -n 1)
        cat < /dev/null > /dev/null

        if [ -z "$FOUND" ]; then
            fail "File not found in $SNAPSHOT_ARG/source: $FILE_PART"
        fi
    ...
    status)
        ...
        done < <(find . -maxdepth 1 \( -name "*.cpp" -o -name "*.h" \) -type f | cat)
        ...
        for f in "${ALL_FILES[@]}"; do
             # Find in snapshot
             FOUND=$(find "$SNAPSHOT_ARG/source" -name "$f" 2>/dev/null | head -n 1)
    ...
    mergetool)
        ...
        FOUND=$(find "$SNAPSHOT_ARG/source" -name "$LOCAL_FILE" | head -n 1)
        if [ -z "$FOUND" ]; then
            fail "File not found in $SNAPSHOT_ARG/source: $LOCAL_FILE"
        fi
    ...
    patch)
        ...
        FOUND=$(find "$SNAPSHOT_ARG/source" -name "$TARGET_FILE" | head -n 1)
        if [ -z "$FOUND" ]; then
             fail "File not found in $SNAPSHOT_ARG/source: $TARGET_FILE"
        fi
        ...
        # Try applying patch.
        if patch --merge -i "$PATCH_FILE" "$TARGET_FILE"; then
    ...
    compile_)
        ...
        SEARCH_DIRS=()
        ALL_FLAGS="${CXXFLAGS:-} ${POLYFILL}"
        # Collapse exactly one space
        ALL_FLAGS="${ALL_FLAGS//-I /-I}"
        ALL_FLAGS="${ALL_FLAGS//-include /-include}"
        ...
        for inc in $INCLUDES; do
            ...
            if [ -z "$RESOLVED" ] && [ -d "snapshot" ]; then
                 FOUND=$(find "$SNAPSHOT_ARG/source" -name "$inc_base" -print -quit | cat)
                 if [ -n "$FOUND" ]; then
                     RESOLVED="$FOUND"
                 fi
            fi
    ...
    compile)
        ...
        for FILE in "${FILES[@]}"; do
            if [ -f "$FILE" ]; then
                ...
                echo LIB= $clangpp_exe @"env/compile.rsp" $POLYFILL $HEADER_OPTS ${CXXFLAGS:-} -c "$FILE" -o "$OBJ" >&2
                LIB= $clangpp_exe @"env/compile.rsp" $POLYFILL $HEADER_OPTS ${CXXFLAGS:-} -c "$FILE" -o "$OBJ"
            fi
        done
        ;;
    ...
    link)
        ...
        mapfile -t LOCAL_OBJS < <(find build -type f -name "*.obj" | cat)
...
EOF
        ...
        ${PYTHONEXECUTABLE} env/filter_llobjs.py "${LOCAL_OBJS[@]}"
        ...
        echo "LIB= $clangpp_exe @env/link.rsp @env/llobjs_filtered.rsp ${LOCAL_OBJS[@]} ${POLYFILL_ARGS[@]} ${LDFLAGS:-} -o $OUTPUT_EXE"
        LIB= $clangpp_exe @"env/link.rsp" @"env/llobjs_filtered.rsp" "${LOCAL_OBJS[@]}" "${POLYFILL_ARGS[@]}" ${LDFLAGS:-} -o "$OUTPUT_EXE"
