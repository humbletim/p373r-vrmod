    ...
    init)
        ...
        if [[ -s "$WINSDK_DIR/vfsoverlay.json" ]] ; then
            jq -s ".[0] * { roots: (.[0].roots + .[1].roots) }" \
                ...
                "$WINSDK_DIR/vfsoverlay.json" > winsdk/_vfsoverlay.json
        else
            cp -av "$REPO_ROOT/experiments/winsdk.in/vfsoverlay.extra.json" winsdk/_vfsoverlay.json
        fi
